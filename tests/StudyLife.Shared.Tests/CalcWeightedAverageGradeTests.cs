...
public class CalcWeightedAverageGradeTests
{
    ...
    {
        Assert.Null(StudyMetrics.CalcWeightedAverageGrade(Array.Empty<StudyMetrics.GradedCourse>()));
    }

    [Fact]
    public void SingleCourse_ReturnsItsGrade()
    {
        var courses = new[] { new StudyMetrics.GradedCourse(2.0m, 5) };
    ...
    public void MultipleCourses_ReturnsEctsWeightedAverage()
    {
        ...
        var courses = new[] { new StudyMetrics.GradedCourse(1.0m, 5), new StudyMetrics.GradedCourse(3.0m, 10) };
    ...
    {
        var courses = new[] { new StudyMetrics.GradedCourse(1.0m, 0), new StudyMetrics.GradedCourse(3.0m, 0) };
        var result = StudyMetrics.CalcWeightedAverageGrade(courses);
    ...
    public void MixOfZeroAndNonZeroEcts_UsesEctsWeightingWhenSumPositive()
    {
        ...
        // (1.0*5 + 4.0*0) / 5 = 1.0
        var courses = new[] { new StudyMetrics.GradedCourse(1.0m, 5), new StudyMetrics.GradedCourse(4.0m, 0) };
        var result = StudyMetrics.CalcWeightedAverageGrade(courses);
    ...
    public void AllSameGrade_ReturnsThatGradeRegardlessOfWeights()
    {
        var courses = new[]
        {
            new StudyMetrics.GradedCourse(2.5m, 5), new StudyMetrics.GradedCourse(2.5m, 10), new StudyMetrics.GradedCourse(2.5m, 30),
        };
