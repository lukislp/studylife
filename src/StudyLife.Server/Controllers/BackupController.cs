...
public class BackupController : ControllerBase
{
    ...
    public BackupController(IBackupDataService backupData,
    ...
    public async Task<IActionResult> DownloadDatabase()
    {
        ...
        try
        {
            ...
            await _backupData.TouchLastBackupDownloadAsync();
    ...
    /// <summary>
    /// Human-readable JSON export of the actual user data ("v2" format) - see
    /// BackupDataService.BuildExportAsync for which tables it carries and which it deliberately
    /// leaves out. Serialized here rather than by the framework's output formatter because the
    /// response is a downloadable File(), not a plain ActionResult&lt;T&gt; - with the SAME
    /// JsonSerializerOptions every other endpoint uses (_jsonOptions), which is the fix for the
    /// divergent-casing bug (audit finding M4(a)).
    /// </summary>
    [Microsoft.AspNetCore.RateLimiting.EnableRateLimiting(RateLimitPolicies.Expensive)]
    [HttpGet("export")]
    public async Task<IActionResult> Export()
    {
        var export = await _backupData.BuildExportAsync();
        // Serialized with the SAME JsonSerializerOptions the framework itself uses for every
        // other endpoint (_jsonOptions) - the fix for the casing bug (audit finding M4(a)).
    ...
    /// BackupDataService.ImportJsonAsync for the id remapping, the ordering it forces and the
    /// tolerance for references the file can't resolve. SessionOnly, but deliberately NOT
    /// owner-gated like the six raw endpoints above: this only ever touches the caller's own
    /// rows (same reasoning as GET .../export already being open to every user) - a real passkey
    /// session is still required so a bare API key (e.g. Home Assistant's) can't wipe an account
    /// on its own.
    /// </summary>
    ...
    public async Task<ActionResult<BackupImportResponseDto>> ImportJson([FromBody] BackupExportDto import)
    {
        ...
        // than a raw aborted connection. Stays in the controller - it is a property of the
        // request, not of the document the service is handed.
        ...
        var result = await _backupData.ImportJsonAsync(import, HttpContext.SessionAuthUserId()!.Value);
        // The one rejection the import itself can produce keeps its original object-shaped body
        // ({ error: ... }), which the plain ToActionResult mapping would flatten to a string.
        return result.Outcome == ServiceOutcome.Invalid
