...
/// <see cref="PlannerController"/> keeps only route binding, the rate-limit policy and
/// status-code mapping.
/// </summary>
public interface IExamPlanService
{
    Task<ServiceResult<List<StudySessionDto>>> GenerateAsync(ExamPlanRequestDto request);
}
...
public class ExamPlanService(
    ...
    ICurrentUserAccessor currentUser) : IExamPlanService
{
    ...
    public async Task<ServiceResult<List<StudySessionDto>>> GenerateAsync(ExamPlanRequestDto request)
    {
        if (request.ExamDate.Date <= DateTime.Today)
            return ServiceResult<List<StudySessionDto>>.Invalid("ExamDate must be in the future.");
        ...
        if (request.ExamDate.Date > DateTime.Today.AddYears(MaxYearsAhead))
            return ServiceResult<List<StudySessionDto>>.Invalid($"ExamDate must be within the next {MaxYearsAhead} years.");
        if (request.SessionLengthMinutes is > 0 and (< MinSessionLengthMinutes or > MaxSessionLengthMinutes))
            return ServiceResult<List<StudySessionDto>>.Invalid($"SessionLengthMinutes must be between {MinSessionLengthMinutes} and {MaxSessionLengthMinutes}.");
        if (request.TotalHours is > MaxTotalHours)
            return ServiceResult<List<StudySessionDto>>.Invalid($"TotalHours must be at most {MaxTotalHours}.");

        var settings = await db.Settings.FirstOrDefaultAsync() ?? new UserSettingsEntity();

        var course = await courseResolver.ResolveInActiveProgramAsync(request.CourseId, settings);
        if (course == null)
            return ServiceResult<List<StudySessionDto>>.Invalid($"Course ID {request.CourseId} is not in the active study program (/api/courses).");
        var goal = await db.CourseGoals.FirstOrDefaultAsync(g => g.CourseId == request.CourseId);
        ...
        _ = webhooks.PublishEventAsync(currentUser.AuthUserId, WebhookEventTypes.PlanGenerated,
            new { courseId = course.Id, courseName = course.Name, sessionsCreated = created.Count, examDate = request.ExamDate },
            CancellationToken.None);

        return ServiceResult<List<StudySessionDto>>.Success(created.Select(e => new StudySessionDto
        {
            Id = e.Id,
            CourseId = e.CourseId,
