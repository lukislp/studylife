...
public class StudyLifeDb : DbContext
{
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        // Optimistic concurrency for the ONE table that the API and the worker both write while
        // the worker is holding its read across an outbound APNs call - see
        // TimerStateEntity.RowVersion for the full reasoning and for why this is a plain int
        // instead of a provider-specific rowversion. BumpConcurrencyTokens below supplies the
        // new value; EF puts the ORIGINAL one into every UPDATE's WHERE clause, so a write that
        // raced another one affects 0 rows and surfaces as DbUpdateConcurrencyException instead
        // of silently overwriting it.
        modelBuilder.Entity<TimerStateEntity>().Property(s => s.RowVersion).IsConcurrencyToken();
    ...
    /// <summary>
    /// Advances the optimistic-concurrency token of every row that is about to be UPDATEd.
    /// Centralized here for the same reason as StampAuthUserIdOnAddedEntries below: a token
    /// that a single write site forgets to bump is worse than no token at all (the WHERE clause
    /// would still match and the conflict would go undetected), and the write sites are spread
    /// across a controller and a background job. Only Modified entries are touched, so this can
    /// never turn a no-op SaveChanges into a real UPDATE; inserts start at 0 (the column
    /// default), which is also what the migration backfills onto existing rows.
    /// </summary>
    private void BumpConcurrencyTokens()
    {
        foreach (var entry in ChangeTracker.Entries<TimerStateEntity>())
        {
...
public class TimerStateEntity
{
    ...
    /// <summary>
    /// Real optimistic-concurrency token (EF IsConcurrencyToken, see OnModelCreating) - unlike
    /// UserSettingsEntity.Version above, which is deliberately only an app-level precondition.
    /// The difference is who the racing writers are: this row is written by the API AND by the
    /// worker, and the worker (RunLiveActivityPushAsync) holds its read across an outbound APNs
    /// HTTP call that can take seconds. In that window the client can stop/pause the timer or
    /// register a fresh live-activity token, and the worker's follow-up write would silently
    /// resurrect the phase it computed from the now-stale row (or null out the brand-new push
    /// token). Since worker and web run as separate processes/pods (k8s/05-worker.yaml), no
    /// in-process synchronization can close that window - only a WHERE clause on the row's own
    /// version can, which is exactly what the token adds to every UPDATE.
    ///
    /// Deliberately a plain int rather than a provider-specific rowversion: the repo maintains
    /// migrations for BOTH SQLite and Postgres from ONE model (see StudyLifeDbPostgres), and
    /// `xmin`/`IsRowVersion()` only exists on one of them. StudyLifeDb.BumpConcurrencyTokens
    /// increments it centrally on every modified row, so no write site can forget to.
    /// </summary>
